...
import { verifyAdminTwoFactorToken, resendAdmin2FACode } from "@/lib/auth";
...
export default function Verify2FAPage() {
  ...
  useEffect(() => {
    ...
    const checkPendingData = () => {
      ...
      const email = sessionStorage.getItem("admin_pending_2fa_email");
      const user = sessionStorage.getItem("admin_pending_2fa_user");
  ...
  const handleVerifyDirect = async (token) => {
    ...
    try {
      const result = await verifyAdminTwoFactorToken(userEmail, token);
  ...
  const handleEnable2FADirect = async (token) => {
    try {
      ...
      if (result.success) {
        ...
        if (typeof window !== "undefined") {
          sessionStorage.removeItem("admi_temp_2fa_secret");
        }
  ...
  const handleResendCode = async () => {
    ...
    try {
      const tempToken = sessionStorage.getItem("admin_temp_auth_token");
      const result = await resendAdmin2FACode(userEmail, tempToken);
